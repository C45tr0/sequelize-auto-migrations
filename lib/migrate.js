...
{ 
    if (typeof defaultValue === 'object') {
        if (defaultValue.constructor && defaultValue.constructor.name && _.has(Sequelize.DataTypes, defaultValue.constructor.name)) {
...
{
    // Backwards compatability (Pulled from https://github.com/sequelize/sequelize/blob/v4/lib/dialects/abstract/query-generator.js#L481-L490)
    // This has been removed in Sequelize V5
    if (idx.indexName) {
        idx.name = idx.indexName;
    }
    if (idx.indicesType) {
        idx.type = idx.indicesType;
    }
    if (idx.indexType || idx.method) {
        idx.using = idx.indexType || idx.method;
    }

    // These seem to be all the available options
    idx.options = _.pick(idx, ['prefix', 'unique', 'type', 'concurrently', 'name', 'using', 'operator', 'parser', 'where']);
